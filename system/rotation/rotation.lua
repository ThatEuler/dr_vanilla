...
function dark_addon.rotation.register(config)
  ...
  if config.combatco then
    setfenv(config.combatco, dark_addon.environment.env)
  end
  if config.resting then
    setfenv(config.resting, dark_addon.environment.env)
  end
  if config.restingco then
    setfenv(config.restingco, dark_addon.environment.env)
  end
...
function dark_addon.rotation.load(name)
  ...
  if rotation then
    ...
    if dark_addon.settings.fetch("netload_rotation_release", nil) then
      dark_addon.log('Loaded rotation: ' .. name .. ' (network)')
    else
      ...
      if not rank_cache[spell_id] then
        rank_cache[spell_id] = setmetatable({
          ...
          __concat = function(l, r)
            if l.namerank then return l.namerank .. r end
            if r.namerank then return l .. r.namerank end
          end
        })
      end
  ...
  if not loading_wait then
    timer = C_Timer.NewTicker(0.3, function()
      if dark_addon.protected then
        ...
        local netload_rotation_release = dark_addon.settings.fetch('netload_rotation_release', false)
        if active_rotation and netload_rotation_release then
          if dark_addon.adv_protected then
            RotationLoader:LoadRotationEWT(active_rotation, netload_rotation_release)
          elseif dark_addon.luabox then
            RotationLoader:LoadRotationLB(active_rotation, netload_rotation_release)
          end

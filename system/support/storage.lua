...
dark_addon.event.register('ADDON_LOADED', function(addonid)
    if addonid == dark_addon.id then
...
function dark_addon.settings.store_toggle(key, value)
    ...
    dark_addon.console.debug(5, 'settings', 'purple', string.format('%s <= %s', full_key, tostring(value)))
    return true
end
...
function dark_addon.settings.fetch_toggle(key, default)
    ...
    if not string.find(full_key, 'master_toggle') then
        dark_addon.console.debug(5, 'settings', 'purple', string.format(

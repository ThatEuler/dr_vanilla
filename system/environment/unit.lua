...
local function channeling(spell)
    local channeling_spell = ChannelInfo(unit.calledUnit.unitID)
...
function unit:castingpercent()
    local castingname, startTime, endTime
    castingname, _, _, startTime, endTime = CastingInfo("player")
...
local function unit_interrupt(percent, spell)
    ...
    name, _, _, startTime, endTime, _, _, notInterruptible, _ = UnitCastingInfo(unit.calledUnit.unitID)
    if not name then
        name, _, _, startTime, endTime, _, _, notInterruptible, _ = UnitChannelInfo(unit.calledUnit.unitID)
    end
...
local function unit_in_range(spell)
    ...
    return IsSpellInRange(spell, unit.calledUnit.unitID) == 1
end
...
local function unit_dispellable(spell)
    return disp:CanDispelWith(unit.calledUnit.unitID, spell)
end
